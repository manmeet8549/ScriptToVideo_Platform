...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // If key is provided in body, test that key. Otherwise, retrieve stored organization key.
    if (key && key.trim() !== '') {
      rawKey = key.trim();
    } else {
      ...
      if (!stored) {
        return NextResponse.json({ success: false, message: 'No key stored for this provider at organization level' }, { status: 400 });
      }

      try {
        rawKey = decrypt(stored.value);
      } catch (decryptionError) {
        console.error('Decryption failed for stored organization key:', decryptionError);
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        console.error(`Fetch error for organization ${providerUpper}:`, fetchError);
    ...
    console.error('[ORG-KEYS TEST] Error:', error);
    return NextResponse.json({ error: 'Failed to test organization provider key' }, { status: 500 });
  }
}
