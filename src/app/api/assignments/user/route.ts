...
export async function GET() {
  ...
  try {
    ...
    const signedAssignments = await Promise.all(
      assignments.map(async (assignment) => {
        ...
        try {
          ...
          console.error(`[USER_ASSIGNMENTS] Failed to sign original video assets:`, err);
        }
        ...
        const signedEditedVideos = await Promise.all(
          assignment.editedVideos.map(async (edited) => {
            ...
            try {
              ...
              console.error(`[USER_ASSIGNMENTS] Failed to sign edited video assets:`, err);
            }
            return {
              ...edited,
              editedVideoUrl,
    ...
    console.error('[ASSIGNMENTS/USER] Error:', error);
